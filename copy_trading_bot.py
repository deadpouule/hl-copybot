...
TOP_X_TRADERS = 5
# Expanded to include PAX, XAG, WTI
ALLOWED_COINS = ["BTC", "ETH", "SOL", "BNB", "PAX", "XAG", "WTI", "HYPE"]
RISK_POS_PCT = 0.10        # 10% of balance per trade
MIN_ORDER_USD = 11.0       
...
class NadoQuantBot:
    def __init__(self):
        logging.info("Initializing Final Perps-Only Master Engine...")
        try:
        ...
        # PRE-LOADED PERP IDs (Correct for Nado Mainnet)
        # BTC is 2, ETH is 3 on Nado for Perps. ID 1 is Spot.
        self.product_map = {
            "BTC": {"id": 2, "p_tick": Decimal("0.1"), "s_tick": Decimal("0.0001")},
            "ETH": {"id": 3, "p_tick": Decimal("0.01"), "s_tick": Decimal("0.001")},
            "SOL": {"id": 4, "p_tick": Decimal("0.001"), "s_tick": Decimal("0.01")},
            "BNB": {"id": 5, "p_tick": Decimal("0.01"), "s_tick": Decimal("0.01")},
            "HYPE": {"id": 100, "p_tick": Decimal("0.001"), "s_tick": Decimal("0.1")}
        }
    ...
    async def sync_nado_memory(self):
        """Wipes memory of trades closed manually on the website."""
        while self.running:
            try:
                sub_info = await asyncio.to_thread(self.client.context.engine_client.get_subaccount_info, self.subaccount_hex)
                live_coins = {str(p.symbol).split('-')[0].upper() for p in getattr(sub_info, 'perp_positions', []) if abs(float(getattr(p, 'amount', 0))) > 1e-8}
                to_clear = [c for c in self.bot_state["positions"].keys() if c not in live_coins]
                for c in to_clear:
                    logging.info(f"Sync: {c} was closed externally. Cleaning memory.")
    ...
    async def sync_market_data(self):
        """Discovers rules and ensures only PERPETUALS are used."""
        while self.running:
            try:
