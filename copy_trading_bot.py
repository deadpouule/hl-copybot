...
class NadoQuantBot:
    def __init__(self):
        ...
        # VERIFIED PERP IDs: Loaded immediately. 
    ...
    async def sync_nado_memory(self):
        while self.running:
            try:
                ...
                for c in to_clear:
                    logging.info(f"Sync: {c} closed on site. Cleaning bot memory.")
    ...
    async def sync_market_data(self):
        """Background rules sync. Non-blocking."""
        while self.running:
            try:
                ...
                async with self.session.get("https://api-evm.orderly.org/v1/public/futures") as r:
                    if r.status == 200:
                        ...
                        for row in js.get("data", {}).get("rows", []):
                            ...
                            # STRICT MATCH: Only match exact Perpetual symbol
