...
class NadoQuantBot:
    def __init__(self):
        logging.info("Initializing Master Nado Copy-Trading Engine...")
        try:
        ...
        # VERIFIED MAINNET PERP IDs (Corrected for Nado: BTC=2, ETH=3)
        self.product_map = {
    ...
    async def sync_nado_memory(self):
        """Auto-Reconciler: Wipes memory of trades closed manually on the site."""
        while self.running:
            try:
    ...
    async def sync_market_data(self):
        ...
        while self.running:
            try:
                ...
                logging.info(f"Market IDs verified: {list(self.product_map.keys())}")
            except: pass
    ...
    async def execute_nado_order(self, coin: str, is_buy: bool, trader: str, is_close: bool):
        ...
        try:
            ...
            # 4. Construct EXACT X18 Parameters per User Reference
            ...
            # Per Documentation: uses SubaccountParams for sender
            sender_params = SubaccountParams(subaccount_owner=NADO_OWNER_ADDR or self.owner, subaccount_name="default")

            order = OrderParams(
                sender=sender_params, priceX18=px_x18, amount=amt_x18,
