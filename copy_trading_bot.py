...
class NadoQuantBot:
    def __init__(self):
        ...
        # VERIFIED PERP IDs: BTC is 2, ETH is 3 on Nado Mainnet.
        # These are loaded immediately so the bot is ARMED at second 1.
        self.product_map = {
    ...
    async def sync_nado_memory(self):
        ...
        while self.running:
            try:
                ...
                for c in to_clear:
                    logging.info(f"Manual Closure: Removing {c} from memory.")
    ...
    async def sync_market_data(self):
        """Background task to update rules without blocking startup."""
        while self.running:
            try:
    ...
    async def orderly_mids_loop(self):
        """Oracle Price Watcher with STRICT matching (No more 0.43 ETH)."""
        while self.running:
            try:
                async with self.session.get("https://api-evm.orderly.org/v1/public/futures") as r:
                    if r.status == 200:
                        ...
                        for row in js.get("data", {}).get("rows", []):
                            ...
                            # STRICT MATCH: Only match exact Perpetual symbols
    ...
    async def run(self):
        ...
        # LAUNCH EVERYTHING AT ONCE (NO BLOCKING)
