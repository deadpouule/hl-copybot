...
class NadoQuantBot:
    def __init__(self):
        ...
        self.orderly_prices = {} # STRICT PRICE CACHE
        self.all_mids = {}
        ...
        # 3. NADO MAINNET MAPPING (Pre-verified)
        self.product_map = {}
    ...
    async def sync_market_data(self):
        """Fetches symbols and maps them STRICTLY to PERP_COIN_USDC format."""
        while self.running:
            try:
                ...
                async with self.session.get(url, timeout=10) as r:
                    if r.status == 200:
                        ...
                        for s in symbols:
                            ...
                            # NADO FORMAT: PERP_ETH_USDC
                            if sym_name.startswith('PERP_') and sym_name.endswith('_USDC'):
                                coin = sym_name.split('_')[1]
                                if coin in ALLOWED_COINS:
                                    ...
                                    self.product_map[coin] = {
                                        ...
                                        "full_symbol": sym_name,
                                        "p_tick_x18": Decimal(s.get('price_increment_x18')), 
                                        "s_tick_x18": Decimal(s.get('size_increment_x18')), 
                                        "min_usd": float(min_usd or 10.0)
                                    }
                        if self.product_map:
                            logging.info(f"Sync SUCCESS: {len(self.product_map)} Nado symbols mapped.")
                            return True
            except: pass
            await asyncio.sleep(10)

    async def orderly_mids_loop(self):
        """Oracle Price Watcher with STRICT symbol matching to prevent collision."""
        while self.running:
            try:
                async with self.session.get("https://api-evm.orderly.org/v1/public/futures") as r:
                    if r.status == 200:
                        ...
                        for row in js.get("data", {}).get("rows", []):
                            ...
                            # Only accept exact PERP_COIN_USDC matches
                            for coin, meta in self.product_map.items():
                                if symbol == meta["full_symbol"]:
    ...
    async def execute_nado_order(self, coin: str, is_buy: bool, trader: str, is_close: bool):
        ...
        try:
            # 1. Price Check (STRICT MATCHED)
            px = self.orderly_prices.get(coin.upper(), 0.0)
            if px == 0:
                logging.warning(f"Price for {coin} not matched yet. Skipping signal.")
                return
            ...
            # X18 Construction (Absolute Integer Safety)
    ...
    async def run(self):
        ...
        # BLOCKING STARTUP: Guaranteed to map strictly before trading
